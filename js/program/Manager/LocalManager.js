...
export default class LocalManager{
  ...
  saveFile(type, data){
    ...
    var self = this;
    if(typeof savedata.location == 'undefined'){
      return;
    }
    fs.writeFile(savedata.location, json, 'utf-8', function(err){
      if(err){
   ...
   */
  saveAllBanks(){
    this.saveFile('all-banks', this.main.dataManager.banksToSaveFile());
  }
  ...
  loadAllBanks(){
    ...
    this.readFile(location[0], function(json){
      if(json){
        if(json.type == 'all-banks'){
          self.main.dataManager.banksFromSaveFile(json.data);
        }else{
          self.main.eventManager.triggerEvent(new ShowPopupEvent('warning', 'Please select a file that contains banks!'));
        }
      }
    });
  }
  ...
  loadCurrentBank(){
    ...
    this.readFile(location[0], function(json){
      if(json){
        if(json.type == 'single-bank'){
          self.main.dataManager.bankFromSaveFile(json.data);
        }else{
          self.main.eventManager.triggerEvent(new ShowPopupEvent('warning', 'Please select a file that contains a bank!'));
        }
      }
    });
  }
